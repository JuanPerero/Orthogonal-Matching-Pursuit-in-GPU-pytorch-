...
// Modelo de archivo utilizado para la depuración de kernels CUDA
...
__global__ void MultiplicationKernel(Types * d_A, Types * d_B, Types * d_R, int N, int M, int fdim) {
    ...
    if (idx == 0) {
        printf("Kernel MultiplicationKernel iniciado - M=%d, N=%d, fdim=%d\n", M, N, fdim);
    }
    
    if (idx < M){
        if (idx == 0) {
            printf("Procesando thread %d\n", idx);
        }
...
void step_cholesky_kernel(Types * d_A, Types * d_B, Types * d_R, int N, int M, int fdim) {
    ...
    printf("EJECUTANDO EL KERNEL - Dimensiones: N=%d, M=%d, fdim=%d\n", N, M, fdim);
    ...
    printf("Configuración del kernel: bloques=%d, hilos=%d\n", blocksPerGrid, threadsPerBlock);
    ...
    printf("Kernel completado exitosamente\n");
}
